...
class PlannerAStar(Planner):
    ...
    def planning(self, start=(100,200), goal=(375,520), inter=None, img=None):
        ...
        while(1):
            ...
            if utils.distance(current_node, goal) < 10:
                self.goal_node = goal
                self.parent[self.goal_node] = current_node
                break
            ...
            for n in neighbors:
                ...
                if self._check_collision(neighbor_node):
                    continue
                if neighbor_node not in self.g:
                    self.g[neighbor_node] = self.g[current_node] + utils.distance(current_node, neighbor_node)
                ...
                elif self.g[neighbor_node] > self.g[current_node] + utils.distance(current_node, neighbor_node):
                    self.g[neighbor_node] = self.g[current_node] + utils.distance(current_node, neighbor_node)
                    self.parent[neighbor_node] = current_node
                    self._queue_insert(neighbor_node)
