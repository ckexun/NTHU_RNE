...
class PlannerRRTStar(Planner):
    ...
    def _near_nodes(self, new_node, search_radius=50):
        near_nodes = []
        for node in self.ntree:
            if utils.distance(node, new_node) < search_radius:
    ...
    def planning(self, start, goal, extend_len=None, img=None):
        ...
        for it in range(20000):
            ...
            near_nodes = self._near_nodes(new_node, 200)
            best_parent, min_cost = self._best_parent(new_node, near_nodes)
            if best_parent == None:
                best_parent = near_node
                min_cost = self.cost[near_node] + utils.distance(near_node, new_node)
            ...
            for near in near_nodes:
                ...
                new_cost = self.cost[new_node] + utils.distance(new_node, near)
                if new_cost < self.cost[near]:  
                    if not self._check_collision(new_node, near):
                        self.ntree[near] = new_node
