...
def mouse_click(event, x, y, flags, param):
    global pose, nav_pos, way_points, path, m_cspace, set_controller_path, show_achieved
...
def render_path(img, nav_pos, way_points, path):
    ...
    if target is not None:
        cv2.circle(img, pos_int(target[:2]), 3, (0,1,0), 4)
    return img

def check_achieved(pose, nas_pos, len_erroe=5):
    d = np.array([pose[0], pose[1]]) - np.array([nas_pos[0], nas_pos[1]])
    distance = np.hypot(d[0], d[1])
    # pose跟nav_pos距離相差5
    if distance < len_erroe:
        if show_achieved:
            print("\nArrived the nav_pos!", end="\t")
            print("Gola:", nav_pos)
            print("Achieved pose:", pose)
...
def navigation(args, simulator, controller, planner, start_pose=(100,200,0)):
    global pose, nav_pos, way_points, path, set_controller_path, show_achieved, target
    # Initialize
    ...
    while(True):
        ...
        if path is not None and collision_count == 0:
            ...
            if check_achieved(pose, nav_pos, 10):
                show_achieved = False
                if args.simulator == "basic":
                    command = ControlState("basic", 0, 0)
                ...
                info = {"x":pose[0], "y":pose[1], "dt":simulator.dt, "yaw":pose[2], 
                        "v":simulator.state.v, "l":simulator.l}
                info["delta"] = simulator.cstate.delta if args.simulator == "bicycle" else None
                if args.simulator == "basic":
                    next_v = 0
                    next_w = 0
                    ...
                    next_v = 10  
                    next_w, target = controller.feedback(info)
                    next_w = np.deg2rad(next_w)
                    next_lw = next_v/r-(next_w*l/r)
                    next_rw = next_v/r+(next_w*l/r)
        ...
        if collision_count > 0:
            ...
            for _ in range(reverse_steps):
                if args.simulator == "basic":
                    ...
                    command = ControlState("bicycle", 5, 0) 
                simulator.step(command)
            ...
            way_points = planner.planning((simulator.state.x, simulator.state.y), nav_pos, 20)
            if len(way_points) > 1:
                path = np.array(cubic_spline_2d(way_points, interval=4))
                set_controller_path = True
                collision_count = 0  
